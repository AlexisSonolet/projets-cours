...
from transform import Trackball, identity, vec, rotate
...
class Mesh:
    ...
    def __init__(self, shader, attributes, index=None):
        self.shader = shader
        names = ['view', 'projection', 'model', 'normal_matrix']
...
class PhongMesh(Mesh):
    ...
    def __init__(self, shader, attributes, index=None,
                 light_position=(0, 10, 0),   # point light (in world coords)
        ...
        names = ['light_position', 'k_a', 's', 'k_s', 'k_d', 'w_camera_position']
    ...
    def draw(self, projection, view, model, primitives=GL.GL_TRIANGLES):
        ...
        GL.glUniform3fv(self.loc['light_position'], 1, self.light_position)
    ...
    def update(self, elapsed_time):
        if self.rotate_light:
            ...
            light_position = rotate((1, 0, 1), angle) @ vec(*self.light_position, 1)
            # self.light_position = light_position[0:3]
...
def main():
    ...
    light_position = (4, 15, 4)
    viewer.add(*[mesh for file in sys.argv[1:]
                 for mesh in load_phong_mesh(file, shader, light_position)])
